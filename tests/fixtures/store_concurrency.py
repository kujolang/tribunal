...
''')
 ...
 try:
  ...
  # A long historical path can reach the byte bound before the entry bound.
  near={'schemaVersion':'1.0.0','runId':'fixture','currentVersion':versions[0]['version'],'versions':[dict(versions[0])]}
  near['versions'][0]['path']='x'*(4194304-len(json.dumps(near).encode())-100)
  index_path.write_text(json.dumps(near)); original=index_path.read_bytes(); assert len(original)<4194304
  bounded=subprocess.run([binary,'run',str(worker),'--interpreter'],env={**env,'RACE_NAME':'c','RACE_EXPECTED':near['currentVersion']},capture_output=True,text=True,timeout=60)
  assert bounded.returncode==0, bounded.stdout+bounded.stderr
  refusal=json.loads(bounded.stdout.strip().splitlines()[-1]); assert not refusal['ok'] and 'safety limit' in refusal['error'], refusal
  assert index_path.read_bytes()==original
  print(json.dumps({'indexByteLimitRefused':True,'versionLimitRefused':True,'conditionalRetryHistory':2,'successfulWriters':sum(r['ok'] for r in results),'historyEntries':len(index['versions']),'initialObjectVersions':1,'finalObjectVersions':len(list((root/'store/objects/fixture').iterdir())),'results':results}))
 finally:
  for p in procs:
