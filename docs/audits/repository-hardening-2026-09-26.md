...
- Ending implementation revision: `a31f660127c8c30c0ebd7ef12ec418aa6cd5ec11` (compatibility runtime installer); application follow-up: `f8febe9`; runtime-target update: `0ab868d072a3ee536f28858f9dbdd2f2aefeec4e`. This report is a subsequent documentation commit; resolve its containing revision with `git log -1 -- docs/audits/repository-hardening-2026-09-26.md`.
...
## Final local verification, application revision f8febe9

At clean report revision `4cfd2cacbd12e179b2491f19a54649e4410be0a7`, official Kujo 1.5 passes all **388 checks**, all 77 source checks, and **95/99 commands**. The four failures are retained: large hearing 95,504 ms / scale pagination 8,039 ms; index pagination 5,054 ms / stats 16,114 ms; lock throughput 12 cycles/sec over 38,861 ms; and the existing local integration-revision mismatch. These results do not satisfy their unchanged budgets. Earlier passing measurements are not substituted for this final local result. Host-load attribution remains unproven; no universal performance claim is made.

Separate exact-commit Kennel validation, archive build, extracted archive smoke and diff check all pass. Their commands, durations, exit codes and log digests are in `directory-release-final` in the JSON receipt. No further source change is justified merely to make a benchmark green. Hosted checks at the final PR revision are required before merge-ready claims; a failed hosted gate remains a blocker. This final documentation update changes only the audit record, not application code.

## Compatibility runtime installation

Compatibility jobs now test the official Kujo 1.5 native release archives, verified against platform-specific SHA-256 pins bound to release commit `cc2d7dbb59a8dc05f00d629e100932f56f4062f6`. This avoids recompiling the unchanged upstream runtime for every compatibility job. The adjacent source checkout remains pinned for matrix verification. **The release workflow still performs its locked Kujo source build, and every Tribunal suite, gate, budget, Eval case, Kennel check and archive smoke check remains in compatibility CI.** No failed benchmark or test was removed or weakened.

`scripts/install_ci_runtime.py` checks matrix agreement, bounds archive and executable sizes, verifies the entire archive before reading executable bytes, ignores archive paths rather than extracting them, refuses links/duplicate executables, verifies the executable version, and publishes without overwriting an existing destination. Seven offline Python regressions cover checksum tampering, archive/executable bounds, regular-file selection, existing-file preservation and wrong-version cleanup. All seven pass. A real local install of the official macOS x64 asset passes and produces the same executable digest used in the official local tests: `3e1e475ea165c8b4a714495596fe8661ad970b27119ad44f1db4d9779f7f05d4`.

The final validation target remains the released runtime requested by the user. Earlier source-built platform and local failures stay in this report; final hosted checks must pass on the exact final PR revision. No universal timing claim or source-build certification is inferred from using a release artifact.

## Hosted closeout

At exact pushed revision `7c3cd2b21f458355e1c95c6f69a5de35b60835cd`, both [push run 36221197855](https://github.com/kujolang/tribunal/actions/runs/36221197855) and [PR run 36221199769](https://github.com/kujolang/tribunal/actions/runs/36221199769) pass all three platforms and the artifact guards. Each platform runs **388 Kujo checks, seven Python installer tests, all fourteen gates, all 20 Eval cases, Kennel validation, doctor, release archive/smoke and diff checks**. Downloaded platform receipts bind that exact source revision to official Kujo 1.5.0, its release source commit and the architecture-specific executable digest.

The [hosted closeout receipt](repository-hardening-2026-09-26-hosted.json) preserves platform records, measured results, exact run references and retained log digests. Initial load-gate observations on the final push are 73 cycles/sec on Linux, 16 on ARM macOS and 21 on Intel macOS, all against the unchanged floor of 15. Those are environment-specific results; the failed local measurements remain in the report and are not retrospectively passed.

The Docker closeout probe still fails because the daemon socket is unavailable. Workcell proof, provider idempotency, the existing native-exclusive-directory proposal and the Concord follow-up remain separate limitations/follow-ups. The native directory proposal is already tracked by `cap_2d4cbc80-9f25-4f5f-ba98-9af41671d983` / `sig_13091122-1d33-4585-8d0c-18d4d6db7119`; no duplicate was created. This receipt-only follow-up changes no application, installer, workflow, test or budget.
