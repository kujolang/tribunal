...
Important permissions include `runs.create`, `runs.read`, `runs.verify`, `runs.sign`, `runs.ingest`, `runs.delete`, `governance.hold`, `keys.generate`, `contracts.validate`, `bundles.export`, `bundles.import`, `store.publish`, `store.pull`, `telemetry.export`, `dashboard.export`, and `locks.recover`.
