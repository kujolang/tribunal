...
The index is a rebuildable projection, not sealed evidence. `index-check` compares both the cached manifest and its digest against storage and rejects duplicate run IDs across shards. Use `index-repair` after a failed check. Index reads, writes, rebuilds and repairs now share an exclusive `<storage>/.index.lock` directory. Each listing call sees a consistent page set; separate cursor calls are not a snapshot across concurrent changes. Acquisition waits up to 30 seconds, then fails explicitly. No lock is reclaimed by age. A failed mutation leaves `<storage>/.index-dirty`; ordinary operations refuse it until `index-repair` or `index-rebuild` succeeds.
