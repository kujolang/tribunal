...
## Governance and telemetry failure recovery

Deletion validates the published retention and external legal-hold schemas. Missing deadlines, malformed history, unsafe files and oversized metadata are refusals, including with `--force-expired`. Hold updates preserve the prior record on validation failure and refuse growth beyond the existing 4 MiB read limit. Preserve a full history for operator reconciliation rather than truncating it.

If deletion fails after its tombstone is written, the tombstone remains `deleting`, the command fails with reconciliation guidance, and the run lock is released when cleanup succeeds. Preserve that evidence, run `index-repair`, and reconcile any remaining run files and tombstone before retrying; an existing tombstone prevents automatic repeat deletion.

JSONL CLI export and telemetry export refuse missing, unsafe, oversized, malformed or non-object event logs. A first-page failure writes no telemetry payload. Later failures retain already exported pages and report failure with the page count and diagnostic; consumers must check `ok` before treating an export as complete. A valid empty log remains supported. Replay also refuses unreadable or malformed events after integrity verification. The best-effort `load_events` helper retains its empty-list failure behavior; completeness-sensitive consumers use `load_events_checked`.
