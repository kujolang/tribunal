...
The September 22 audit adds full secret screening of constructed context before persistence or model calls, suppresses rejected response bodies in blind-stage failures, validates encrypted aggregate/inventory metadata before decryption, and bounds secondary contract/receipt readers. Reference-store route guards cover cleanup and reads as well as uploads. These regressions do not replace the commissioned independent review.

Replay idempotency keys are computed locally, but the current AI SDK bridge does not forward them to a provider. Full-stage replay can repeat live provider requests and charges; provider-side deduplication is not a supported guarantee.
