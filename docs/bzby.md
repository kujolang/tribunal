...
Dashboards should use run IDs and content hashes as join keys, preserve stopped/partial records, distinguish mock from live evidence, and never ingest raw environment values. Tribunal v0.7.0 includes a dependency-free offline dashboard export for local inspection; it still embeds no BZBY or network-service dependency.
